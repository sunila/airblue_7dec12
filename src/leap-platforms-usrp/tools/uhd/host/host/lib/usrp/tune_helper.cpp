...
static tune_result_t tune_xx_subdev_and_dsp(
    ...
    if (1){
        ...
        if (bw > rate) lo_offset = 12e6;//std::min((bw - rate)/2, rate/2);
        printf("Rate: %f, bw: %f, lo_offset: %f\n", rate, bw, lo_offset);
    } else {
      printf("Did not adjust lo\n");
    }
    ...
    printf("Tune result target_inter_freq %f actual_inter_freq %f target_dsp_freq %f actual_dsp_freq %f\n", target_inter_freq, actual_inter_freq, target_dsp_freq, actual_dsp_freq);
    return tune_result;
}
...
static double derive_freq_from_xx_subdev_and_dsp(
    ...
    printf("Tune result actual_dsp_freq %f actual_inter_freq %f , delta  %f\n", actual_inter_freq, actual_dsp_freq,actual_inter_freq - actual_dsp_freq);
